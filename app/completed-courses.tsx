...
export default function CompletedCourses() {
  ...
  const completedCourses = usersCourse?.filter((course) => course.status === "completed")

  const handleCreatePress = (course: userCourseProp) => {
    setSelectedCourse(course);
    setModalVisible(true);
  };

  const filteredCourses = (completedCourses || []).filter(item => 
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {/* List */}
      <ScrollView contentContainerStyle={[styles.listContainer, filteredCourses.length === 0 && styles.emptyList]}>
        {filteredCourses.length > 0 ? (
          filteredCourses.map((item) => (
            <View key={item.id} style={styles.card}>
              <Image 
                source={{ uri: item.course.thumbnail || 'https://images.unsplash.com/photo-1550751827-4bd374c3f58b?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&q=80' }} 
                style={styles.courseImage} 
              />
              <View style={styles.cardContent}>
                <Text style={[styles.courseTitle, { color: colors.text }]}>{item.course.title}</Text>
                <Text style={[styles.instructor, { color: colors.gray300 }]}>{item.course.instructor?.name || 'Instructor'}</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={[styles.emptyText, { color: colors.text }]}>
              {searchQuery ? "No matching courses found" : "No completed courses yet"}
            </Text>
            <Text style={[styles.emptySubtext, { color: colors.gray300 }]}>
              {searchQuery ? "Try searching with a different term" : "Complete your enrolled courses to see them here"}
            </Text>
          </View>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  emptyContainer: {
    ...
    marginTop: 20, // Move it further up
  },
  emptyText: {
    fontSize: 18,
    fontWeight: '600',
